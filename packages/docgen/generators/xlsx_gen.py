...
class XlsxGenerator(DocumentGenerator):
    ...
    def generate(
        ...
        brand = brand or {}
        # Derive header fill color from brand primary_color (strip '#')
        raw_color = brand.get("primary_color", "#4472C4").lstrip("#").upper()
        header_fill = PatternFill(start_color=raw_color, end_color=raw_color, fill_type="solid")
        ...
        if len(sections_with_tables) <= 1:
            ...
            self._write_single_sheet(ws, content, header_fill, header_font)
        else:
            ...
            for section in sections_with_tables:
                ...
                self._write_section_sheet(ws, section, header_fill, header_font)
    ...
    # ------------------------------------------------------------------
    def _write_single_sheet(self, ws, content: DocumentContent, header_fill, header_font) -> None:
        row = 1
        cell = ws.cell(row=row, column=1, value=content.title)
        cell.font = _TITLE_FONT
        row += 2
        ...
        for section in content.sections:
            ...
            if section.table:
                row = self._write_table(ws, section.table, row, header_fill, header_font)
                row += 1
            elif section.body:
                for line in section.body.split("\n"):
                    stripped = line.strip()
    ...
    def _write_section_sheet(self, ws, section: DocumentSection, header_fill, header_font) -> None:
        row = 1
        if section.body:
            for line in section.body.split("\n"):
                stripped = line.strip()
        ...
        if section.table:
            self._write_table(ws, section.table, row, header_fill, header_font)

    def _write_table(self, ws, table_data: list[list[str]], start_row: int, header_fill=None, header_font=None) -> int:
        if not table_data:
            return start_row

        hf = header_fill if header_fill is not None else _HEADER_FILL
        hft = header_font if header_font is not None else _HEADER_FONT
